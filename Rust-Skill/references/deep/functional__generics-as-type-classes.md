> **来源**:本文逐字移植自《Rust Design Patterns》(https://github.com/rust-unofficial/patterns) 的 `src/functional/generics-type-classes.md`,许可 **MPL-2.0**。
...
- If a type seems to need a "split API" due to construction or partial
  ...
  [Builder Pattern](./pattern__builder.md) instead.

- If the API between types does not change -- only the behavior does -- then the
  [Strategy Pattern](./pattern__strategy.md) is better used
  instead.
