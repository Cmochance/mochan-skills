> **来源**:本文逐字移植自《Rust Design Patterns》(https://github.com/rust-unofficial/patterns) 的 `src/patterns/ffi/export.md`,许可 **MPL-2.0**。
...
- [Wrapper Type Consolidation](./ffi__type-consolidation-wrappers.md) groups multiple Rust types
  together into an opaque "object"

- [FFI Error Passing](./ffi__error-handling.md) explains error handling with
  integer codes and sentinel return values (such as `NULL` pointers)

- [Accepting Foreign Strings](./ffi__accepting-strings.md) allows
  accepting strings with minimal unsafe code, and is easier to get right than
  [Passing Strings to FFI](./ffi__passing-strings.md)
