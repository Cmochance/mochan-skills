...
- **若要抓 proxy wire(forward-trace 验上游 200/400):transfer 必须以诊断模式启动**(`CAS_DIAG_TRACE=1`)—— 诊断默认关、重启即关,普通启动抓不到 trace。
...
# 1. 带诊断起 transfer(CAS_DIAG_TRACE=1 → forward-trace 才写)。先杀旧 transfer 避免单实例锁
...
- **授权**:为跑测试可自行 pkill+open(用户 2026-06-17 放宽);非测试场景仍遵守"build 完不自动 open"。
