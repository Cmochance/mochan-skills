...
- **关键全局结构**:
  ...
  - `UClass` / `UProperty`(UE 4.25+ 改为 `FProperty`,不再继承 `UObject`)/ `UFunction` — 描述类布局、字段偏移、蓝图可调函数。
- **思路(SDK / dumper)**:
...
- **常见坑**:
  - 偏移随 UE 大版本(4.x ↔ 5.x)及小版本变化,UE 4.25 把多数 `UProperty` 改成 `FProperty`(不再是 `UObject`),旧 dumper 直接挂。
...
- **常见检测维度(概念)**:
  - **内核态驱动**:部分反作弊(如 Vanguard、EAC/BattlEye 的内核组件)运行在 Ring 0,监控驱动加载、句柄打开、内存读写来源,检测面远超用户态。加载时机有别:Vanguard 是 boot-start 驱动,开机随系统内核早期加载;EAC/BattlEye 的内核驱动则在游戏启动时加载、退出时卸载。
...
- **网络协议观察**:
  - 抓包:Wireshark 看原始流量,mitmproxy / Fiddler / Charles / Burp 类工具看 HTTP(S);TLS 加密需在自有/授权环境配信任证书。
